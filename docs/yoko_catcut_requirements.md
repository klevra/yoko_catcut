...
- 웹페이지 좌측 하단에 배경 캐릭터 이미지를 표시한다.
- 이미지 경로는 `frontend/background.png`이다.
- 권장 이미지는 정사각형 `300px x 300px` PNG이다.
- 배경 캐릭터는 50% 투명도로 표시한다.
- 배경 캐릭터는 클릭, 입력, 편집 기능을 방해하지 않아야 한다.
- 테스트 기본 이미지는 스마일 캐릭터를 사용한다.
